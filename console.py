...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, line):
        """Catching commands if nothing else matches."""
        # print("DEF:::", line)
        self._precmd(line)

    def _precmd(self, line):
        """Intercepting commands in order to test for class.syntax()"""
        # print("PRECMD:::", line)
    ...
    def update_dict(self, classname, uid, s_dict):
        """Helper method to update() with dictionary."""
        s = s_dict.replace("'", '"')
        d = json.loads(s)
        if not classname:
    ...
    def do_count(self, line):
        """Counting the instances of a class.
        """
        words = line.split(' ')
        if not words[0]:
            print("** class name missing **")
    ...
    def do_update(self, line):
        """Updating an instance by adding or updating attribute.
        """
        if line == "" or line is None:
            print("** class name missing **")
            return
